...
int partition(vector<Bid>& bids, int begin, int end) {
	...
	while (!done) {
		...
		if (low >= high) {
			...
			// Swap the low and high with the built-in vector method
...
void quickSort(vector<Bid>& bids, int begin, int end) {
	...
	// Partition bids into low and high pairs
	mid = partition(bids, begin, end);

	// Recursively call quickSort using midpoint value (begin to mid)
	quickSort(bids, begin, mid);

	// Recursively call quickSort using midpoint value (mid + 1 and the end)
...
int main(int argc, char* argv[]) {
    ...
    case 2:
        ...
        break;
    default:
        csvPath = "./Debug/eBid_Monthly_Sales_Dec_2016.csv";
    }
